...
COMMENT ON COLUMN situation.situation_id IS 'Идентификатор ситуации';
...
CREATE TABLE IF NOT EXISTS cause (
    ...
    situation_id UUID REFERENCES situation(situation_id) ON DELETE CASCADE,
    description  VARCHAR(100)
);

COMMENT ON TABLE cause IS 'Причины (родитель для advice, дочерняя для situation)';
COMMENT ON COLUMN cause.cause_id IS 'Идентификатор причины';
...
CREATE TABLE IF NOT EXISTS advice (
    ...
    cause_id    UUID REFERENCES cause(cause_id) ON DELETE CASCADE,
    description VARCHAR(50)
);

COMMENT ON TABLE advice IS 'Рекомендации (дочерняя таблица для cause)';
COMMENT ON COLUMN advice.advice_id IS 'Идентификатор рекомендации';

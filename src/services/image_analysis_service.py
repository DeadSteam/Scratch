...
from .exceptions import ValidationError as ServiceValidationError, NotFoundError
...
class ImageAnalysisService:
    ...
    def calculate_histogram(self, grayscale_image: np.ndarray) -> Dict[int, int]:
        """
    ...
    def analyze_image(self, image_data: bytes, rect_coords: Optional[List[float]] = None) -> Dict:
        """
        ...
        try:
            ...
            raise ServiceValidationError("PIL (Pillow) is required for image processing")
    ...
    async def analyze_experiment_images(
        ...
        for scratched_id in scratched_image_ids:
            ...
            if scratched_image.experiment_id != experiment_id:
                raise ServiceValidationError(f"Image {scratched_id} does not belong to this experiment")
            ...
            scratch_index = self.calculate_scratch_index(
                scratched_analysis["histogram"],
                scratched_analysis["total_pixels"]
            )
        ...
        reference_scratch_index = self.calculate_scratch_index(
            reference_analysis["histogram"],
            reference_analysis["total_pixels"]
        )

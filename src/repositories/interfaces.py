...
class UserRepositoryInterface(BaseRepository[T], ABC):
    ...
    @abstractmethod
    async def get_by_username(self, username: str, session: AsyncSession) -> Optional[T]:
        """Get user by username."""
        pass
    
    @abstractmethod
    async def get_by_email(self, email: str, session: AsyncSession) -> Optional[T]:
        """Get user by email."""
        pass
    
    @abstractmethod
    async def get_active_users(self, session: AsyncSession, skip: int = 0, limit: int = 100) -> List[T]:
...
class ExperimentRepositoryInterface(BaseRepository[T], ABC):
    ...
    @abstractmethod
    async def get_by_user_id(self, user_id: UUID, session: AsyncSession, skip: int = 0, limit: int = 100) -> List[T]:
        """Get experiments by user ID."""
        pass
    
    @abstractmethod
    async def get_by_film_id(self, film_id: UUID, session: AsyncSession, skip: int = 0, limit: int = 100) -> List[T]:
        """Get experiments by film ID."""
        pass
    
    @abstractmethod
    async def get_by_config_id(self, config_id: UUID, session: AsyncSession, skip: int = 0, limit: int = 100) -> List[T]:
        """Get experiments by config ID."""
        pass
    
    @abstractmethod
    async def get_with_images(self, id: UUID, session: AsyncSession) -> Optional[T]:

...
from ..core.dependencies import ImageAnalysisSvc, ExperimentSvc, MainDBSession
...
class ScratchAnalysisRequest(BaseModel):
    ...
    scratched_image_ids: List[UUID] = Field(..., min_length=1, description="List of scratched image IDs")
...
class ScratchAnalysisResponse(BaseModel):
    ...
    reference_image: dict = Field(..., description="Reference image analysis")
    scratched_images: List[ScratchedImageResult] = Field(..., description="Scratched images analysis")
    ...
    if save_to_experiment:
        ...
        for img in result["scratched_images"]:
            scratch_results.append({
                ...
                "scratch_index": img["scratch_index"]
            })
...
async def get_image_histogram(
    ...
    rect_coords = experiment.rect_coords if experiment and hasattr(experiment, 'rect_coords') else None
